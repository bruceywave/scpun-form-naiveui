<template>
  <div class="scpun-widget-form__container">
    ...
    <n-form :label-width="widgetForm.config.lableWidth" :show-require-mark="widgetForm.config.hideRequiredMark"
      ...
      <Draggable class="widget-form-list" item-key="key" ghostClass="ghost" handle=".drag-widget" :animation="200"
        :group="{ name: 'default' }" :list="widgetForm.list" @add="handleMoveAdd">

        <template #item="{ element, index }">
          <TransitionGroup name="fade" tag="div">
            <template v-if="element.type === 'grid'">
              <n-row class="widget-col widget-view">
                <n-col v-for="(col, colIndex) of element.columns" :key="colIndex" :span="col.span ?? 0">
                  <Draggable class="widget-col-list" item-key="key" ghostClass="ghost" handle=".drag-widget"
                    :animation="200" :group="{ name: 'default' }" :no-transition-on-drag="true" :list="col.list"
                    @add="handleColMoveAdd($event, element, colIndex)">
                    <template #item="{ element, index }">
                      <transition-group name="fade" tag="div">
                        <FormWidgetPaneItem v-if="element.key" :key="element.key" :element="element"
                          :config="widgetForm.config" :select-widget="widgetFormSelect"
                ...
                <div class="widget-view-action widget-col-action" v-if="widgetFormSelect?.key === element.key">
                  <SvgIcon type="delete" @click.stop="handleDeleteClick(index, widgetForm.list)" />
                </div>
            ...
            <template v-else>
              <FormWidgetPaneItem v-if="element.key" :key="element.key" :element="element" :config="widgetForm.config"
                :select-widget="widgetFormSelect" @click.stop="handleItemClick(element)"
